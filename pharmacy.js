...
export class Pharmacy {
  static SignedDrug = Object.freeze({
    HERBT: "Herbal Tea",
    FERX: "Fervex",
    MPILL: "Magic Pill",
    DAFAL: "Dafalgan",
  ...
  drugSignedLowerBenefit(drug) {
    const noneLower = [
      Pharmacy.SignedDrug.HERBT,
      Pharmacy.SignedDrug.FERX,
      Pharmacy.SignedDrug.MPILL
    ];
    if (drug.benefit == 0 || noneLower.includes(drug.name) == true) {
      return;
    }
  ...
  drugSignedIncreaseBenefit(drug) {
    ...
    if (
      drug.benefit == Pharmacy.SignedDrug.max_benef ||
    ...
    if (drug.name == Pharmacy.SignedDrug.FERX) {
      drug.benefit +=
        drug.benefit < Pharmacy.SignedDrug.max_benef && drug.expiresIn < 11
          ? 1
          : 0;
      drug.benefit +=
        drug.benefit < Pharmacy.SignedDrug.max_benef && drug.expiresIn < 6
          ? 1
          : 0;
    }
